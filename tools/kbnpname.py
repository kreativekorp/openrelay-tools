...
try:
	kbnpUnicodeTable = {v: k for k, v in kbnpNameTable.items()}
except:
	kbnpUnicodeTable = {v: k for k, v in kbnpNameTable.iteritems()}
...
def kbnpName(cp):
	...
	if cp < 0 or cp >= 0x110000:
		return '.notdef'
	if cp in kbnpNameTable:
		return kbnpNameTable[cp]
	if cp < 0x10000:
		return 'uni%04X' % cp
	return 'u%05X' % cp
...
def kbnpNames(s):
	try:
		return [kbnpName(cp) for cp in unicode(s, 'utf8')]
	except:
		return [kbnpName(cp) for cp in s]

def kbnpUnicode(name):
	if name in kbnpUnicodeTable:
		return kbnpUnicodeTable[name]
...
def kbnpResolve(arg):
	try:
		if type(arg) is int:
			...
			cp = int(arg)
		return (cp, kbnpName(cp))
	except:
		return (kbnpUnicode(arg), arg)

def main():
	for arg in sys.argv[1:]:
		cp, name = kbnpResolve(arg)
		if cp < 0:
			print('%d\t%s' % (cp, name))
		else:
